...
target_task = sys.argv[1] if len(sys.argv) > 1 else 'su26_fe_re'
...
item = next((x for x in index_data if x['id'] == target_task), None)
if not item:
    print(f"Task {target_task} not found in index.")
    sys.exit(0)

task_json_path = os.path.join(hcm_dir, f"{target_task}.json")
if not os.path.exists(task_json_path):
    print(f"File {task_json_path} not found.")
    sys.exit(0)

print(f"Initializing EasyOCR Reader for {target_task}...")
...
for idx, img_info in enumerate(hd_images):
    q_num = idx + 1
    filename = f"{target_task}_{q_num:03d}.webp"
    ...
    if os.path.exists(hd_path):
        try:
            ...
            if ocr_lines:
                ...
                print(f"[{target_task}] Q{q_num} updated: {q_obj['question'][:40]}...")
        except Exception as e:
            pass

print(f"✓ FINISHED {target_task}.json ({updated_count}/{len(hd_images)} Qs updated)")
