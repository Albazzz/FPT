...
print("Initializing Robust EasyOCR Engine...")
reader = easyocr.Reader(['vi'], gpu=False, verbose=False)

def clean_junk(s):
    s = s.strip()
    s = re.sub(r'^(?:Câu\s*\d+|Question\s*\d+|\d+|Choose|answer|Multiple|Choice|hoode|neet|aneet|aneeg|ree\[|ree|[\s\[\]])+', '', s, flags=re.I).strip()
    s = re.sub(r'\s*(?:7Exam|FUExam|Exam|EXT|EXQ[A-Z]*|thống)\s*$', '', s, flags=re.I).strip()
    return s
...
def parse_hd_perfect(lines):
    ...
    for line in lines:
        s = line.strip()
        if s in ('Choose', 'answer', 'Multiple Choice', 'hoode') or re.match(r'^(?:7Exam|FUExam|Exam|thống|EXT)$', s, re.I):
            continue
        cleaned.append(s)
    ...
    for x in cleaned[:second_a]:
        if not re.match(r'^(?:Câu|Question|\d+|[A-D]|Choose|answer|Multiple|Choice|hoode|neet|ree\[)$', x, re.I):
            q_tokens.append(x)
    q_text = ' '.join(q_tokens).strip()
    q_text = clean_junk(q_text)
    ...
    return q_text, clean_junk(' '.join(opts['A'])), clean_junk(' '.join(opts['B'])), clean_junk(' '.join(opts['C'])), clean_junk(' '.join(opts['D']))
...
print(f"\nFINISHED ALL HD OCR PARSING! (Total {total_updated_overall} Qs updated)")
