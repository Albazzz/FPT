...
task_id = sys.argv[1] if len(sys.argv) > 1 else 'su26_fe_re'
...
hd_images = item.get('hdImages', []) if item else []

print(f"Initializing EasyOCR for {task_id} ({len(hd_images)} HD images)...")
...
for idx, img_info in enumerate(hd_images):
    ...
    if os.path.exists(hd_path):
        ...
        if ocr_lines:
            ...
            updated_count += 1
            print(f"[{task_id}] Q{q_num}: {q_obj['question'][:40]}... (A: {a_text[:20]}...)")
...
print(f"✓ FINISHED {task_id}.json ({updated_count}/{len(hd_images)} Qs updated)")
